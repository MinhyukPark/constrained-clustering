...
int MinCutGlobalClusterRepeat::main() {
    ...
    while (true) {
        ...
        int seed = 0;
        node_id_to_cluster_id_map = ConstrainedClustering::GetCommunities("", this->algorithm, seed, this->resolution, &graph);
        ConstrainedClustering::RemoveInterClusterEdges(&graph, node_id_to_cluster_id_map);
    ...
    this->WriteClusterQueue(MinCutGlobalClusterRepeat::to_be_clustered_clusters);
    return 0;
}
